  ...
  useEffect(() => {
    const fetchMarketingMap = async () => {
      ...
      try {
        ...
        if (!res.ok) throw new Error(`Failed to load marketing map (${res.status})`)
        const json = (await res.json()) as { mapping?: Record<string, string>; displays?: Record<string, string> }
        setMarketingMap(json?.mapping ?? {})
        setMarketingDisplays(json?.displays ?? {})
        console.log(`Marketing map is ${JSON.stringify(json?.mapping)}`)
        console.log(`Marketing displays is ${JSON.stringify(json?.displays)}`)
      } catch (err: any) {
